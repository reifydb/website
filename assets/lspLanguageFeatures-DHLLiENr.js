import { R as Ge, l as x, e as w, U as Je, M as R } from "./index-B5EvIA7f.js";
var T;
(function(e) {
  function n(i) {
...
(function(e) {
  function n(r, t, a, u) {
    if (o.uinteger(r) && o.uinteger(t) && o.uinteger(a) && o.uinteger(u)) return { start: _.create(r, t), end: _.create(a, u) };
    if (_.is(r) && _.is(t)) return { start: r, end: t };
...
(function(e) {
  function n(r, t, a, u) {
    return { red: r, green: t, blue: a, alpha: u };
  }
  e.create = n;
  function i(r) {
    const t = r;
...
(function(e) {
  function n(r, t, a, u, s, f) {
    let l = { name: r, detail: t, kind: a, range: u, selectionRange: s };
...
function it(e, n) {
  ...
  return { severity: rt(n.severity), startLineNumber: n.range.start.line + 1, startColumn: n.range.start.character + 1, endLineNumber: n.range.end.line + 1, endColumn: n.range.end.character + 1, message: n.message, code: i, source: n.source };
}
class mt {
  constructor(n, i) {
  ...
  provideCompletionItems(n, i, r, t) {
    ...
    return this._worker(a).then((u) => u.doComplete(a.toString(), k(i))).then((u) => {
      ...
      const s = n.getWordUntilPosition(i), f = new Ge(i.lineNumber, s.startColumn, i.lineNumber, s.endColumn), l = u.items.map((d) => {
        const g = { label: d.label, insertText: d.insertText || d.label, sortText: d.sortText, filterText: d.filterText, documentation: d.documentation, detail: d.detail, command: ut(d.command), range: f, kind: ot(d.kind) };
        return d.textEdit && (at(d.textEdit) ? g.range = { insert: v(d.textEdit.insert), replace: v(d.textEdit.replace) } : g.range = v(d.textEdit.range), g.insertText = d.textEdit.newText), d.additionalTextEdits && (g.additionalTextEdits = d.additionalTextEdits.map(C)), d.insertTextFormat === J.Snippet && (g.insertTextRules = x.CompletionItemInsertTextRule.InsertAsSnippet), g;
      });
      return { isIncomplete: u.isIncomplete, suggestions: l };
    });
  }
}
function k(e) {
...
function Ye(e) {
  if (e) return { start: { line: e.startLineNumber - 1, character: e.startColumn - 1 }, end: { line: e.endLineNumber - 1, character: e.endColumn - 1 } };
}
function v(e) {
...
class vt {
  ...
  provideHover(n, i, r) {
    ...
    return this._worker(t).then((a) => a.doHover(t.toString(), k(i))).then((a) => {
      if (a) return { range: v(a.range), contents: ct(a.contents) };
    });
  }
}
function st(e) {
...
function Qe(e) {
  return typeof e == "string" ? { value: e } : st(e) ? e.kind === "plaintext" ? { value: e.value.replace(/[\\`*_{}[\]()#+\-.!]/g, "\\$&") } : { value: e.value } : { value: "```" + e.language + `
` + e.value + "\n```\n" };
}
function ct(e) {
...
class bt {
  ...
  provideDocumentHighlights(n, i, r) {
    ...
    return this._worker(t).then((a) => a.findDocumentHighlights(t.toString(), k(i))).then((a) => {
      if (a) return a.map((u) => ({ range: v(u.range), kind: dt(u.kind) }));
    });
  }
}
function dt(e) {
...
function Ze(e) {
  return { uri: Je.parse(e.uri), range: v(e.range) };
}
class xt {
  constructor(n) {
    this._worker = n;
...
function ft(e) {
  ...
  for (let i in e.changes) {
    const r = Je.parse(i);
    for (let t of e.changes[i]) n.push({ resource: r, versionId: void 0, textEdit: { range: v(t.range), text: t.newText } });
  }
  return { edits: n };
}
class kt {
  ...
  provideDocumentSymbols(n, i) {
    ...
    return this._worker(r).then((t) => t.findDocumentSymbols(r.toString())).then((t) => {
      if (t) return t.map((a) => lt(a) ? Te(a) : { name: a.name, detail: "", containerName: a.containerName, kind: Ke(a.kind), range: v(a.location.range), selectionRange: v(a.location.range), tags: [] });
    });
  }
}
function lt(e) {
...
function Te(e) {
  return { name: e.name, detail: e.detail ?? "", kind: Ke(e.kind), range: v(e.range), selectionRange: v(e.selectionRange), tags: e.tags ?? [], children: (e.children ?? []).map((n) => Te(n)) };
}
function Ke(e) {
  let n = x.SymbolKind;
  switch (e) {
...
class At {
  ...
  provideLinks(n, i) {
    ...
    return this._worker(r).then((t) => t.findDocumentLinks(r.toString())).then((t) => {
      if (t) return { links: t.map((a) => ({ range: v(a.range), url: a.target })) };
    });
  }
}
class Et {
...
class It {
  ...
  provideDocumentColors(n, i) {
    ...
    return this._worker(r).then((t) => t.findDocumentColors(r.toString())).then((t) => {
      if (t) return t.map((a) => ({ color: a.color, range: v(a.range) }));
    });
  }
...
class Rt {
  ...
  provideFoldingRanges(n, i, r) {
    ...
    return this._worker(t).then((a) => a.getFoldingRanges(t.toString(), i)).then((a) => {
      if (a) return a.map((u) => {
        const s = { start: u.startLine + 1, end: u.endLine + 1 };
...
class Mt {
  ...
  provideSelectionRanges(n, i, r) {
    ...
    return this._worker(t).then((a) => a.getSelectionRanges(t.toString(), i.map(k))).then((a) => {
      if (a) return a.map((u) => {
        const s = [];
        for (; u; ) s.push({ range: v(u.range) }), u = u.parent;
        return s;
      });
    });
  }
